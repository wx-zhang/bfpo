...
class Trainer:
    ...
    def __init__(
        ...
        buffer_dataset: Optional[Dataset] = None,
    ...
    def _inner_training_loop(
        ...
        buffer_dataloader = self.get_train_dataloader(self.buffer_dataset) if self.buffer_dataset is not None else None
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            # setup buffer set 
            if buffer_dataloader is not None:
                buffer_iterator = iter(buffer_dataloader)
            else:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if buffer_iterator:
                    try:
                        inputs_buffer = next(buffer_iterator)
                ...
                with self.accelerator.accumulate(model):
                    tr_loss_step = self.training_step(model, inputs, inputs_buffer)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], inputs_buffer: Dict[str, Union[torch.Tensor, Any]] = None) -> torch.Tensor:
        """
        ...
        if is_sagemaker_mp_enabled():
            ...
            if inputs_buffer is not None:
                loss_bf = smp_forward_backward(model, inputs_buffer, self.args.gradient_accumulation_steps)
                loss = loss_mb.reduce_mean().detach().to(self.args.device) + loss_bf.reduce_mean().detach().to(self.args.device)
            else: 
        ...
        if inputs_buffer is not None: 
            with self.compute_loss_context_manager():
                loss_bf = self.compute_loss(model, inputs_buffer)

            if self.args.n_gpu > 1:
                loss_bf = loss_bf.mean()  # mean() to average on multi-gpu parallel training
            loss += loss_bf

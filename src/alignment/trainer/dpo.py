...
from alignment.trainer.trainer import Trainer
...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            if args.use_selective:
                self.freeze_model(model,args)
        ...
        train_dataset = train_dataset.map(self.tokenize_row,load_from_cache_file=False)
        # train_dataset.map(self.tokenize_row)
        if buffer_dataset is not None: 
            buffer_dataset = buffer_dataset.map(self.tokenize_row,load_from_cache_file=False)
        if eval_dataset is not None:
            eval_dataset = eval_dataset.map(self.tokenize_row,load_from_cache_file=False)
    ...
    def get_train_dataloader(self, dataset: Optional[Dataset] = None) -> DataLoader:
        """
        ...
        return super().get_train_dataloader(dataset)
    ...
    def freeze_model(self,model,args):
        for n,w in model.named_parameters():
            if any([update_module in n for update_module in args.selected_layers]):
